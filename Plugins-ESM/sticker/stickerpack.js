...
import { imageToWebp } from '../../Library/sticker.js';
...
const handler = async (m, { conn, text, usedPrefix, command }) => {
    if (command === 'stickerpack_pick') {
        ...
        try {
            ...
            for (const s of pool) {
                try {
                    const raw = await downloadBuffer(s.image);
                    const webp = s.animated ? raw : await imageToWebp(raw);
...
handler.command = /^(stickerpack|stickerpack_pick)$/i;
handler.limit = true;

 ...
 */
const VERSION = '4.7';
import { generateWAMessageFromContent, prepareWAMessageMedia, generateMessageIDV2 } from '@itsliaaa/baileys';
import crypto from 'crypto';
import sharp from 'sharp';
import ffmpeg from 'fluent-ffmpeg';
import { PassThrough, Readable } from 'stream';
import { set as kvSet } from '../Database/kvstore.js';
const CRM_CACHE_TTL_MS = 3 * 24 * 60 * 60 * 1000; 
...
class Toolkit {
	...
	static async replyRaw(conn, chatId, text, quotedMsg, opts = {}) {
		if (!conn || !chatId || !quotedMsg) {
			throw new Error('replyRaw butuh conn, chatId, dan quotedMsg (pesan yang mau di-quote)');
		}
		const participant = quotedMsg.key?.participant || quotedMsg.key?.remoteJid;
		const stanzaID = quotedMsg.key?.id;
		const quotedContent = quotedMsg.message || { conversation: quotedMsg.text || '' };
		if (!participant || !stanzaID) {
			throw new Error('quotedMsg gak lengkap: butuh key.participant/key.remoteJid dan key.id');
		}
		const contextInfo = {
			participant,
			stanzaID,
		...
		if (opts.expirationSeconds) {
			contextInfo.disappearingMode = { initiator: 'CHANGED_IN_CHAT', trigger: 'UNKNOWN' };
			contextInfo.expiration = opts.expirationSeconds;
		}
		const content = {
			extendedTextMessage: {
				text,
		...
		return await conn.relayMessage(chatId, content, {
...
class Button extends BaseBuilder {
	...
	static STRICT_NATIVE_FLOW_NAMES = ['payment_key_info'];
	async send(jid, { messageId, additionalNodes = [], ...options } = {}) {
		const msg = await this.build(jid, { messageId, ...options });
		const hasCustomBizNode = additionalNodes.some((n) => n?.tag === 'biz');
		const singleButtonName = this._buttons.length === 1 ? this._buttons[0]?.name : null;
		const flowName = singleButtonName && Button.STRICT_NATIVE_FLOW_NAMES.includes(singleButtonName) ? singleButtonName : 'mixed';
...
class Carousel extends BaseBuilder {
	...
	async send(jid, { messageId, additionalNodes = [], ...options } = {}) {
		...
		try {
			kvSet(`crm_cache:${msg.key.id}`, msg.message, CRM_CACHE_TTL_MS);
		} catch (_) {}
		await this.#client.relayMessage(msg.key.remoteJid, msg.message, {
			messageId: msg.key.id,
			additionalNodes: [
